...
lsp_installer.on_server_ready(function(server)
    ...
    -- if server.name == "jsonls" then
      --  local jsonls_opts = require("user.config.lsp.settings.jsonls")
    ...
    if server.name == "tsserver" then
        local tss_ops = require("user.config.lsp.settings.tsserver")
        opts = vim.tbl_deep_extend("force", tss_ops, opts)
    end
